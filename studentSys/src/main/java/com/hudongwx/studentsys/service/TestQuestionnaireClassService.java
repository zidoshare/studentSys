...
public class TestQuestionnaireClassService extends Service {
    ...
    public Page<TestQuestionnaireClass> getByClassId(int page,Class aClass){
        return TestQuestionnaireClass.dao.paginate(page, Common.MAX_PAGE_SIZE,Common.COMMON_SELECT,TestQuestionnaireClass.SQL_FROM +"where classId = ? order by id desc",aClass.getId());
    }

    public Page<TestQuestionnaireClass> getAllTQC(int currentPage){
        return TestQuestionnaireClass.dao.paginate(currentPage,Common.MAX_PAGE_SIZE,Common.COMMON_SELECT,TestQuestionnaireClass.SQL_FROM +Common.ORDER_BY_ID_DESC);
    }
}
